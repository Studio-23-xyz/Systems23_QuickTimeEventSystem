...
using Studio23.SS2.QTESystem.UI;
using System;
...
namespace Studio23.SS2.QTESystem.Core
{
	public class QTEManager : MonoBehaviour
	{
		...
		public async UniTask<List<string>> CheckForQuickTimeInput(QTEDataSO QTEData)
		{
			...
			InputReader = InputSystem.onAnyButtonPress.Call(quickTimePress =>
			{
				...
				{
					Debug.Log(
						$"Button pressed <color=#f426ff> {quickTimePress.name} </color> from device = {quickTimePress.device.name}");
